...
const yesterdaysNotes = computed(() => {
    ...
    return notes.value.filter((note) => {
        const noteData = new Date(note.updatedAt)
        return noteData.toDateString() === yesterday.toDateString()
    })
})
...
const earlierNotes = computed(() => {
    ...
    return notes.value.filter((note) => {
        const noteData = new Date(note.updatedAt)
        return (noteData < yesterday && noteData.toDateString() !== yesterday.toDateString())
    })
})
